...
COMPANIES = [
    ...
    },
    {
        ...
        "state": "Kerala",
        "pin_code": "682002",
        "phone": "+91-484-4001002",
        "email": "nexa@dms.local",
        "gstin": "32NEXA0002Z1",
    },
    {
        "company_name": "Jaguar",
        ...
        "state": "Kerala",
        "pin_code": "682003",
        "phone": "+91-484-4001003",
        "email": "jaguar@dms.local",
        "gstin": "32JAGUAR003Z1",
    },
]
...
VEHICLES = {
    "Honda": [
        ("Honda City ZX", "City", "ZX CVT", "White", 1600000),
        ("Honda Elevate VX", "Elevate", "VX MT", "Silver", 1550000),
        ("Honda Amaze VX", "Amaze", "VX CVT", "Red", 1050000),
    ],
    "NEXA": [
        ("NEXA Baleno Alpha", "Baleno", "Alpha AMT", "Blue", 1120000),
        ("NEXA Fronx Alpha", "Fronx", "Alpha Turbo", "Grey", 1390000),
        ("NEXA Grand Vitara", "Grand Vitara", "Zeta", "White", 1980000),
    ],
    "Jaguar": [
        ("Jaguar XE R-Dynamic", "XE", "R-Dynamic", "Black", 5200000),
        ("Jaguar F-Pace Prestige", "F-Pace", "Prestige", "Blue", 7800000),
        ("Jaguar XF Portfolio", "XF", "Portfolio", "Silver", 6900000),
    ],
}

CUSTOMERS = {
    "Honda": [
        ("Anil Mathew", "anil.honda@example.com", "9000001001"),
        ("Priya Nair", "priya.honda@example.com", "9000001002"),
        ("Rahul Menon", "rahul.honda@example.com", "9000001003"),
    ],
    "NEXA": [
        ("Meera Thomas", "meera.nexa@example.com", "9000002001"),
        ("Arjun Pillai", "arjun.nexa@example.com", "9000002002"),
        ("Sneha Raj", "sneha.nexa@example.com", "9000002003"),
    ],
    "Jaguar": [
        ("Vikram Varma", "vikram.jaguar@example.com", "9000003001"),
        ("Diya Kurian", "diya.jaguar@example.com", "9000003002"),
        ("Kiran George", "kiran.jaguar@example.com", "9000003003"),
    ],
}
...
def _as_datetime(year: int, month: int, day: int = 15) -> str:
    return datetime(year, month, day, 10, 0, 0).strftime("%Y-%m-%d %H:%M:%S")


def _get_existing_name(doctype: str, filters: dict[str, Any]) -> str | None:
    return frappe.db.get_value(doctype, filters, "name")


def _insert_if_missing(doctype: str, filters: dict[str, Any], values: dict[str, Any]) -> str:
    existing = _get_existing_name(doctype, filters)
    if existing:
        return existing
...
def _seed_companies() -> dict[str, str]:
    ...
    for company in COMPANIES:
        ...
        company_ids[company["company_name"]] = name
...
def _seed_customers(company_ids: dict[str, str]) -> dict[str, list[str]]:
    ...
    for company_name, company_customers in CUSTOMERS.items():
        customer_ids[company_name] = []

        for customer_name, email, mobile_no in company_customers:
            customer_id = _insert_if_missing(
                "DMS Customer",
            ...
            customer_ids[company_name].append(customer_id)
...
def _seed_vehicles(company_ids: dict[str, str]) -> dict[str, list[str]]:
    ...
    for company_name, vehicles in VEHICLES.items():
        vehicle_ids[company_name] = []

        for index, (vehicle_name, model, variant, color, price) in enumerate(vehicles, start=1):
            chassis_no = f"{company_name.upper().replace(' ', '')}-CH-{index:03d}"
            engine_no = f"{company_name.upper().replace(' ', '')}-EN-{index:03d}"
            vehicle_id = _insert_if_missing(
                "DMS Vehicle",
                {"chassis_no": chassis_no},
                {
                    ...
                    "color": color,
                    "year": 2026,
                    "fuel_type": "Petrol" if company_name != "Jaguar" else "Diesel",
            ...
            vehicle_ids[company_name].append(vehicle_id)
...
def _seed_sales(company_ids: dict[str, str], customer_ids: dict[str, list[str]], vehicle_ids: dict[str, list[str]]) -> int:
    ...
    for company_name, monthly_values in SALES_BY_COMPANY.items():
        for month_index, amount in enumerate(monthly_values):
            year, month = MONTHS[month_index]
            customer_id = customer_ids[company_name][month_index % len(customer_ids[company_name])]
            vehicle_id = vehicle_ids[company_name][month_index % len(vehicle_ids[company_name])]
            creation = _as_datetime(year, month, day=15)
            ...
            _set_creation("DMS Vehicle Sale", sale_name, creation)
...
def _seed_service_jobs(company_ids: dict[str, str], customer_ids: dict[str, list[str]]) -> int:
    ...
    for company_name, monthly_counts in SERVICE_COUNTS_BY_COMPANY.items():
        for month_index, count in enumerate(monthly_counts):
            year, month = MONTHS[month_index]
            customer_id = customer_ids[company_name][month_index % len(customer_ids[company_name])]

            for sequence in range(1, count + 1):
                creation = _as_datetime(year, month, day=min(25, 1 + (sequence % 25)))
...
def _seed_leads(company_ids: dict[str, str]) -> int:
    ...
    for company_name, lead_titles in LEADS.items():
        for index, title in enumerate(lead_titles, start=1):
            mobile_no = f"91000{list(LEADS).index(company_name) + 1}{index:04d}"
            _insert_if_missing(
                "DMS Lead",
...
def _seed_bookings_and_test_drives(
    company_ids: dict[str, str],
    customer_ids: dict[str, list[str]],
    vehicle_ids: dict[str, list[str]],
) -> dict[str, int]:
    ...
    for company_name in COMPANIES_BY_NAME():
        for index in range(2):
            ...
            _insert_if_missing(
                "DMS Test Drive",
                {"mobile_no": f"92000{list(COMPANIES_BY_NAME()).index(company_name) + 1}{index:04d}", "company_id": company_ids[company_name]},
                {
                    "company_id": company_ids[company_name],
                    "customer_id": customer_id,
                    "contact_name": frappe.db.get_value("DMS Customer", customer_id, "customer_name"),
                    "mobile_no": f"92000{list(COMPANIES_BY_NAME()).index(company_name) + 1}{index:04d}",
                    "vehicle_id": vehicle_id,
                    "model": frappe.db.get_value("DMS Vehicle", vehicle_id, "model"),
...
def _seed_invoices(company_ids: dict[str, str], customer_ids: dict[str, list[str]]) -> int:
    ...
    for company_name in COMPANIES_BY_NAME():
        for index in range(2):
            invoice_no = f"SEED-{company_name.upper()}-INV-{index + 1}"
            customer_id = customer_ids[company_name][index]

            _insert_if_missing(
                "DMS Invoice",
                {"reference_doc": invoice_no, "company_id": company_ids[company_name]},
                {
                    "company_id": company_ids[company_name],
...
def COMPANIES_BY_NAME() -> list[str]:
    return [company["company_name"] for company in COMPANIES]


@frappe.whitelist()
def seed() -> dict[str, Any]:
    """Create repeatable local demo data for the DMS AI dashboard."""
...
def reset() -> dict[str, Any]:
    """Delete only seed/demo records created by this module where possible."""
    ...
    return {
        "status": "success",
        "message": "DMS demo data reset completed.",
        "deleted": deleted,
    }
